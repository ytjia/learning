...
object Huffman {
    ...
    def times(chars: List[Char]): List[(Char, Int)] =
        chars.groupBy(ch => ch).map(charMap => (charMap._1, charMap._2.size)).toList
    ...
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
        def decode0(pTree: CodeTree, remain: List[Bit], acc: List[Char]): List[Char] = (pTree, remain) match {
            case (pTree: Leaf, Nil) => acc ::: chars(pTree)
            case (pTree: Leaf, rest) => decode0(tree, rest, acc :::chars(pTree))
            case (pTree: Fork, 0 :: rest) => decode0(pTree.left, rest, acc)
            case (pTree: Fork, 1 :: rest) => decode0(pTree.right, rest, acc)
        }
    ...
    def convert(tree: CodeTree): CodeTable = {
        def link(chars: List[Char], bit: Bit, acc: CodeTable): CodeTable =
            chars.map( char => (char, codeBits(acc)(char) :+ bit))

        def convert0(pTree: CodeTree, acc: CodeTable): CodeTable = pTree match {
            case p: Fork =>
                mergeCodeTables(convert0(p.left, link(chars(p.left), 0, acc)), convert0(p.right, link(chars(p.right), 1, acc)))
            case p: Leaf => acc
        }
